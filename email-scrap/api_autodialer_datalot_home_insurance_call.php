<?php
	...
	//$phonebook_id=36054;	
	//$phonebook_id=46381;
	$phonebook_id=58178; //DATALOT HOME INSURANCE CALL
	...
	$sqlproperty_Query="select * from scrapped_datalot_new_home_insurance_call where phonebook_status = 0"; // fetching all records where phonebook_status is not updated
	...
	if(mysqli_num_rows($property_Query)>0){
		while ($db_field = mysqli_fetch_assoc($property_Query)) {
			$id					=	$db_field['id'];
			$phone_number		=	$db_field['home_phone'];
			...
			if(!empty($phone_number)){
				...
				if(!empty($response_id)){ // checking whether records added or not.
					// update the table.
					$sql_update="update scrapped_datalot_new_home_insurance_call set phonebook_status = 1 where id='".$id."'";
