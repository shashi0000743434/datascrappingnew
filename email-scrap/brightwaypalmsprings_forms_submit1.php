<?php	
	...
	$sqlproperty_Query="select * from scrapped_emails_2 where webform_status = 0"; // fetching all records where auto_dialer_status is not updated
	//$sqlproperty_Query="select * from scrapped_emails_2 where id =2"; // fetching all records where auto_dialer_status is not updated		
	...
	if(mysqli_num_rows($query)>0){
		...
		while ($db_field = mysqli_fetch_assoc($query)) {	
				...
				$postinfo = "utf8=&#x2713;&authenticity_token=$authenticity_token&person[given_name]=$first_name&person[middle_name]=&person[last_name]=$last_name&person[suffix]=&lead[email]=$email&lead[home]=1&lead[routing_group_id]=FMAP&agent_uid=&api_key=$api_key&from=&commit=Get quote";	
				...
				$postinfo = "utf8=&#x2713;&authenticity_token=$authenticity_token&lead[address_house_number]=&lead[address_street_name]=$street_address&lead[address_city]=$city&lead[address_postal_code]=$postal_code&dwelling[built_dt_year]=&dwelling[use]=&dwelling[construction_type]=&dwelling[occupied_by]=&dwelling[total_sq_ft]=&dwelling[cov]=$schedule_value&nextpage=default&api_key=$api_key2";	
				curl_setopt($ch, CURLOPT_TIMEOUT, 400); //timeout in seconds
				curl_setopt($ch, CURLOPT_URL, $url);
				curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
				curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
				...
				echo '<br>https://entryform.semcat.net/brightwaypalmsprings/personal/finish?api_key='.$api_key2.'<br>';
				...
				if($result_final==1){
					// update the table.
					$sql_update="update scrapped_emails_2 set webform_status = 1 where id='".$id."'";
