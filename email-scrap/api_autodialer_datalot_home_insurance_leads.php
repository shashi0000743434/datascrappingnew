<?php
	...
	$sqlproperty_Query="select * from scrapped_datalot_home_insurance_lead where phonebook_status = 0"; // fetching all records where phonebook_status is not updated
		...
		while ($db_field = mysqli_fetch_assoc($property_Query)) {
			...
			if(!empty($phone_number)){
				...
				if(!empty($response_id)){ // checking whether records added or not.
					// update the table.
					$sql_update="update scrapped_datalot_home_insurance_lead set phonebook_status = 1 where id='".$id."'";
