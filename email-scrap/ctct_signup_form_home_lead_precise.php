<?php
	...
	**/	
	...
	$sqlproperty_Query="select * from scrapped_home_precise_leads where ctct_status = 0"; 	
	//$sqlproperty_Query="select * from scrapped_home_precise_leads where id = 1"; 		
	...
	if(mysqli_num_rows($property_Query)>0){
		while ($db_field = mysqli_fetch_assoc($property_Query)) {		
			...
			$location = explode(',',$db_field['address']);
			if(count($location) == 3) {
				$street_address		=	$location[0].', '.$location[1];
				$city				=	substr($street_address,-2);
				$state				=	substr($street_address,-2);
				$postal_code		=	trim($location[2]);
			}
			elseif(count($location) == 2) {
				$street_address		=	$location[0];
				$city				=	substr($street_address,-2);
				$state				=	substr($street_address,-2);
				$postal_code		=	trim($location[1]);
			}
			elseif(count($location) == 4) {
				$street_address		=	$location[0].', '.$location[1];
				$city				=	substr($street_address,-2);
				$state				=	substr($street_address,-2);
				$postal_code		=	trim($location[3]);
			}
			...
			$postinfo = "ca=e8cffc00-f006-4609-a018-07b95a93b284&list=1671667451&source=EFD&required=list,email,first_name&url=&email=$email&first_name=$first_name&last_name=$last_name&phone=$phone&address_street=$street_address&address_city=$city&address_state=$state&address_postal_code=$postal_code";
			echo '<br>';
			...
			if($response==1){
				...
				// update the table.
				$sql_update="update scrapped_home_precise_leads set ctct_status = 1 where id='".$id."'";
