<?php
	...
	**/	
	...
	$sqlproperty_Query="select * from scrapped_datalot_home_insurance_lead where ctct_status = 0"; 	
	//$sqlproperty_Query="select * from scrapped_datalot_home_insurance_lead where id = 3"; 		
	...
	if(mysqli_num_rows($property_Query)>0){
		while ($db_field = mysqli_fetch_assoc($property_Query)) {		
			...
			if($response==1){
				...
				// update the table.
				$sql_update="update scrapped_datalot_home_insurance_lead set ctct_status = 1 where id='".$id."'";
