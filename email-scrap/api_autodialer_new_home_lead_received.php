<?php
	...
	//$phonebook_id=36054;	
	//$phonebook_id=46949;
	$phonebook_id=58179; //NEW HOME LEAD
	...
	$sqlproperty_Query="select * from scrapped_new_home_lead_received where phonebook_status = 0"; // fetching all records where phonebook_status is not updated
	...
	if(mysqli_num_rows($property_Query)>0){
		while ($db_field = mysqli_fetch_assoc($property_Query)) {
			$id					=	$db_field['id'];
			$phone_number		=	$db_field['daytime_phone'];
			...
			if(!empty($response_id)){ // checking whether records added or not.
				// update the table.
				$sql_update="update scrapped_new_home_lead_received set phonebook_status = 1 where id='".$id."'";
