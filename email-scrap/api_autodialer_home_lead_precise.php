<?php
	...
	$sqlproperty_Query="select * from scrapped_home_precise_leads where phonebook_status = 0"; // fetching all records where phonebook_status is not updated
	...
	if(mysqli_num_rows($property_Query)>0){
		while ($db_field = mysqli_fetch_assoc($property_Query)) {
			...
			if(!empty($response_id)){ // checking whether records added or not.
				// update the table.
				$sql_update="update scrapped_home_precise_leads set phonebook_status = 1 where id='".$id."'";
